  ...
  <div>
    ...
    <div v-else-if="loading" class="grid gap-6 lg:grid-cols-[280px,1fr] animate-pulse">
      <aside class="space-y-4">
        ...
        <div class="rounded-xl border border-white/10 bg-[#14192d] p-4 space-y-2">
          <div class="h-2.5 w-24 rounded-full bg-white/10 mb-3" />
        ...
        <div class="rounded-xl border border-white/10 bg-[#14192d] p-4 space-y-3">
          <div class="h-2.5 w-16 rounded-full bg-white/10" />
          <div class="h-10 rounded-lg bg-white/5" />
          <div class="h-10 rounded-lg bg-white/5" />
        </div>
      </aside>
      <div class="rounded-xl border border-white/10 bg-[#14192d] overflow-hidden">
        ...
        <div v-for="i in 8" :key="i" class="flex items-center gap-3 border-b border-white/10 px-5 py-4">
    ...
    <div v-else>
      ...
      <button
        type="button"
        class="mb-4 flex w-full items-center justify-between rounded-xl border border-white/10 bg-[#14192d] px-4 py-3 text-sm text-white/70 shadow-sm md:hidden"
      ...
      <div class="grid gap-6 md:grid-cols-[280px,1fr] items-start">
        <!-- Sidebar: search, categories, filters — sticky on desktop -->
        <aside :class="['space-y-4 md:sticky md:top-6', sidebarOpen ? 'block' : 'hidden md:block']">
          <!-- Search card -->
          <div class="rounded-xl border border-white/10 bg-[#14192d] p-4 shadow-sm">
            <label for="kb-search" class="sr-only">Search</label>
            <div class="relative">
              <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                <svg aria-hidden="true" class="w-4 h-4 text-white/40" fill="currentColor" viewBox="0 0 20 20">
                  <path
                    fill-rule="evenodd"
              ...
              <input
                ...
                v-model="query"
                type="text"
                class="bg-white/5 border border-white/10 text-white text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full pl-10 p-2.5 placeholder:text-white/50"
          ...
          <!-- Categories card -->
          <div class="rounded-xl border border-white/10 bg-[#14192d] p-4 shadow-sm">
            <div class="text-xs font-semibold uppercase tracking-wide text-white/50 mb-3">Categories</div>
            <nav class="space-y-1">
              <button
                type="button"
              ...
              >
                ...
                <span class="tabular-nums text-xs text-white/40">{{ baseFilteredArticles.length }}</span>
              </button>
              <button
              ...
              >
                <span>{{ cat.name }}</span>
                <span class="tabular-nums text-xs text-white/40">{{ cat.count }}</span>
              </button>
            </nav>
          </div>

          <!-- Filters card -->
          <div class="rounded-xl border border-white/10 bg-[#14192d] p-4 shadow-sm space-y-3">
            <div class="text-xs font-semibold uppercase tracking-wide text-white/50">Filters</div>
            <div>
              <label class="block mb-1 text-xs text-white/50" for="kb-provision">Provision type</label>
              <select
                id="kb-provision"
                v-model="provision"
            ...
            <div>
              <label class="block mb-1 text-xs text-white/50" for="kb-product-version">Product version</label>
              <select
        ...
        <!-- Article list card -->
        <section class="rounded-xl border border-white/10 bg-[#14192d] overflow-hidden shadow-sm">
          <!-- Header row -->
          <div class="flex items-center justify-between border-b border-white/10 px-5 py-4">
