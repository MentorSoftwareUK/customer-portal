...
<template>
  <div class="space-y-6">
    ...
    <!-- Error -->
    <div v-if="loadError" class="rounded-lg border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-300">
      {{ loadError }}
    </div>

    <!-- Skeleton -->
    <div v-else-if="loading" role="status" class="animate-pulse space-y-5">
      <div class="h-48 rounded-lg bg-gray-50" />
      <div class="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_340px]">
        <div class="space-y-4">
          <div class="h-40 rounded-lg bg-gray-50" />
          <div class="h-28 rounded-lg bg-gray-50" />
        </div>
        <div class="space-y-4">
          <div class="h-36 rounded-lg bg-gray-50" />
          <div class="h-32 rounded-lg bg-gray-50" />
        </div>
      </div>
    ...
    <template v-else-if="event">
      ...
      <nav class="flex" aria-label="Breadcrumb">
        <ol class="inline-flex items-center gap-1.5 text-sm">
          <li>
            <RouterLink to="/app/events" class="font-medium text-gray-400 hover:text-gray-600 transition">
              Events
            </RouterLink>
          </li>
          <li class="text-gray-300" aria-hidden="true">/</li>
          <li aria-current="page" class="font-medium text-gray-500 truncate max-w-[220px] sm:max-w-none">
            {{ event.title }}
          </li>
        </ol>
      </nav>
      ...
      <!-- ─── Hero ───────────────────────────────────────────────────────── -->
      <div class="rounded-lg border border-gray-200 bg-white px-6 py-8 text-gray-900 shadow-sm sm:px-8">
        <span
        ...
        <h1 class="mt-4 text-2xl font-semibold tracking-tight leading-tight tracking-tight text-gray-900 sm:text-4xl">
          {{ event.title }}
        </h1>
        ...
        <p v-if="event.description" class="mt-4 text-[0.9375rem] leading-relaxed text-gray-500">
          {{ event.description }}
        </p>
      </div>
      ...
      <div class="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_340px] lg:items-start">
        ...
        <section class="space-y-4">
          ...
          <!-- What we'll cover -->
          <div class="rounded-lg border border-gray-200 bg-gray-50 px-6 py-6 text-gray-900">
            <h2 class="text-base font-semibold tracking-tight text-black">What we'll cover</h2>
            <ul class="mt-4 space-y-3">
              <li class="flex items-start gap-3 text-[0.9375rem] leading-relaxed text-gray-500">
          ...
          <!-- Who should attend -->
          <div class="rounded-lg border border-gray-200 bg-gray-50 px-6 py-6 text-gray-900">
            <h2 class="text-base font-semibold tracking-tight text-black">Who should attend</h2>
            <p class="mt-4 text-[0.9375rem] leading-relaxed text-gray-500">
          ...
          <!-- Session etiquette -->
          <div class="rounded-lg border border-gray-200 bg-gray-50 px-6 py-6 text-gray-900">
            <h2 class="text-base font-semibold tracking-tight text-black">Session etiquette</h2>
            <p class="mt-4 text-[0.9375rem] leading-relaxed text-gray-500">
            ...
            <p class="mt-3 text-[0.9375rem] leading-relaxed text-gray-500">
          ...
          <!-- Need help? -->
          <div class="rounded-lg border border-gray-200 bg-gray-50 px-6 py-6 text-gray-900">
            <h2 class="text-base font-semibold tracking-tight text-black">Need help?</h2>
            <p class="mt-4 text-[0.9375rem] leading-relaxed text-gray-500">
              If you're unsure whether this session is right for your team, register anyway and add a note — we'll follow up.
            </p>
            <p class="mt-3 text-sm text-gray-400">
              <span class="font-medium text-gray-500">Tip:</span> If you can't attend live, we'll share materials and a recording link afterwards when available.
            </p>
          </div>

          <!-- Event resources (post-event) -->
          <div v-if="showResources" class="rounded-lg border border-gray-200 bg-gray-50 px-6 py-6 text-gray-900">
        ...
        <aside class="space-y-4 lg:sticky lg:top-6">
          ...
          <div class="rounded-lg border border-gray-200 bg-white px-5 py-5 text-gray-900 shadow-sm">

            <!-- Registered state — full confident block -->
            <template v-if="isRegistered">
              <div class="rounded-lg border border-emerald-500/30 bg-emerald-500/15 px-5 py-5 text-center">
            ...
            <template v-else-if="isPaymentPending">
              <div class="rounded-lg border border-amber-500/30 bg-amber-500/15 px-4 py-4">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <p class="text-sm font-semibold text-amber-600">Payment pending</p>
              ...
              <RouterLink
                ...
                class="mt-4 inline-flex w-full items-center justify-center rounded-lg bg-[#e7007e] px-4 py-3.5 text-base font-semibold text-white shadow hover:bg-[#c8006c] transition focus:outline-none focus:ring-4 focus:ring-[#e7007e]/30"
              >
                Complete payment
              </RouterLink>
            </template>
            ...
            <template v-else>
              <h2 class="text-sm font-semibold text-gray-400 uppercase tracking-wide">Register</h2>
              <p class="mt-1 text-xl font-semibold tracking-tight text-black">Join this session</p>
              <p class="mt-1 text-sm text-gray-400">{{ priceLabel }}</p>
              <RouterLink
                :to="`/app/events/${event.id}/register`"
                class="mt-5 inline-flex w-full items-center justify-center rounded-lg bg-[#e7007e] px-4 py-3.5 text-base font-semibold text-white shadow-[0_4px_20px_rgba(231,0,126,0.35)] hover:bg-[#c8006c] hover:shadow-[0_4px_20px_rgba(231,0,126,0.5)] transition focus:outline-none focus:ring-4 focus:ring-[#e7007e]/30"
              >
                Register now
              </RouterLink>
            </template>
          ...
          <div class="rounded-lg border border-gray-200 bg-gray-50 px-5 py-5 text-gray-900">
            ...
            <dl class="mt-3 space-y-2.5 text-sm">
              <div class="flex items-start justify-between gap-4">
                ...
                <dd class="text-right font-medium text-gray-700">{{ event.dateLabel }}</dd>
              </div>
              <div class="flex items-start justify-between gap-4">
                <dt class="text-gray-400">Duration</dt>
                <dd class="text-right font-medium text-gray-700">{{ durationLabel }}</dd>
              </div>
              <div class="flex items-start justify-between gap-4">
                <dt class="text-gray-400">Platform</dt>
                <dd class="text-right font-medium text-gray-700">{{ event.platform }}</dd>
              </div>
              <div v-if="event.joinUrl" class="flex items-start justify-between gap-4">
                <dt class="text-gray-400">Join link</dt>
                <dd class="text-right">
                  <a
                    v-if="isRegistered"
                    :href="event.joinUrl"
                  ...
                  <span v-else class="inline-flex items-center gap-1 text-gray-300 text-xs">
              ...
              <div v-if="event.provisionLabel" class="flex items-start justify-between gap-4">
                <dt class="text-gray-400">Provision</dt>
                <dd class="text-right font-medium text-gray-700">{{ event.provisionLabel }}</dd>
              </div>
              <div v-if="event.eligibilityLabel" class="flex items-start justify-between gap-4">
                <dt class="text-gray-400">Eligibility</dt>
                <dd class="text-right font-medium text-gray-700">{{ event.eligibilityLabel }}</dd>
              </div>
            </dl>
          </div>
